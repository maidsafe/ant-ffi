/**
 * JVM Unit tests for Autonomi key derivation functionality. These tests run on the host JVM without
...
class KeyDerivationTest {
  ...
  fun testKeyDerivation() {
    ...
    assertEquals("Derived pubkeys should match", derivedPkFromSk.toHex(), derivedPkFromPk.toHex())
  }
  ...
  fun testSignatureVerification() {
    ...
    assertTrue("Valid signature should verify", mainPk.verify(signature, msg))
  }
  ...
  fun testSignatureVerificationFails() {
    ...
    assertFalse("Wrong key should fail verification", mainSk2.publicKey().verify(signature, msg1))

    // Wrong message should fail
    assertFalse(
        "Wrong message should fail verification", mainSk1.publicKey().verify(signature, msg2))
  }
  ...
  fun testDerivedKeySignature() {
    ...
    assertTrue(
        "Derived signature should verify with derived pubkey", derivedPk.verify(signature, msg))
  ...
  fun testSignatureFromBytes() {
    ...
    assertTrue("Restored signature should verify", mainSk.publicKey().verify(restored, msg))
  }
