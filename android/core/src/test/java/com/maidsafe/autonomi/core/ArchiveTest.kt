/**
 * JVM Unit tests for Autonomi archive functionality. These tests run on the host JVM without
...
class ArchiveTest {
  ...
  fun testClientArchiveMethodsExist() {
    ...
    val expectedMethods =
        listOf("archiveCost", "archiveGetPublic", "archivePutPublic", "archiveGet", "archivePut")

    for (method in expectedMethods) {
      assertTrue("Client should have $method method", methodNames.contains(method))
    }
  }
  ...
  fun testClientFileMethodsExist() {
    ...
    val expectedMethods =
        listOf("fileCost", "fileUpload", "fileUploadPublic", "fileDownload", "fileDownloadPublic")
  ...
  fun testClientDirectoryMethodsExist() {
    ...
    val expectedMethods =
        listOf(
            "dirUpload", "dirUploadPublic", "dirDownload", "dirDownloadPublic", "dirContentUpload")
