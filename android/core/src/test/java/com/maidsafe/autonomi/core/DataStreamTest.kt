 ...
 *
 * Note: DataStream requires network operations for full functionality. These tests verify the API
 * structure and basic operations. Full integration tests require a running Autonomi network.
 */
...
class DataStreamTest {
   ...
   *
   * Note: Full DataStream testing requires network operations. This test verifies the types exist
  ...
  fun testClientStreamingMethodsExist() {
    ...
    assertTrue("Client should have dataStream method", methodNames.contains("dataStream"))
    assertTrue(
        "Client should have dataStreamPublic method", methodNames.contains("dataStreamPublic"))
  }
  ...
  /**
   * Test DataStream class exists via reflection. This ensures the FFI bindings expose the expected
   * class.
   */
  @Test
