 ...
 *
 * These tests verify that private data, files, and archives can be uploaded and downloaded on a
...
class FileNetworkTest {
  ...
  fun testFileUploadDownloadPublic(): Unit = runBlocking {
    try {
      ...
      assertEquals(
          "Downloaded content should match", "Hello from Autonomi!", downloadFile.readText())
  ...
  fun testFileUploadDownloadPrivate(): Unit = runBlocking {
    try {
      ...
      assertEquals(
          "Downloaded content should match", "Secret file content", downloadFile.readText())
