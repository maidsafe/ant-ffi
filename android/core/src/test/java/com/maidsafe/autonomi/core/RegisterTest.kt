/**
 * JVM Unit tests for Autonomi register functionality. These tests run on the host JVM without
...
class RegisterTest {
  ...
  fun testRegisterKeyFromName() {
    ...
    assertEquals(
        "Same name should derive same key", key1.publicKey().toHex(), key1Again.publicKey().toHex())
